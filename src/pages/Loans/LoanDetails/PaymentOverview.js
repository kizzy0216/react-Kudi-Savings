...
export default ({ loan }) => {
  ...
  const nextPayment = nextPaymentRes ? nextPaymentRes.data ? nextPaymentRes.data.data : null : null;
  const paymentsOverview = paymentsOverviewRes ? paymentsOverviewRes.data ? paymentsOverviewRes.data.data : defaultPaymentOverview : defaultPaymentOverview;
  ...
  return <CardBody className={'Payment-History-Card'}>
    ...
    <div className="part-2">
      ...
      <p className={'add-margin-bottom'}><span className="key">Due Date</span> <span className="value">
        {nextPayment ? new Date(nextPayment.expectedRepaymentDate).toISOString().split('T')[0] : '-- --'}
      </span></p>
      <p><span className="key">Payment Count</span> <span className="value">{paymentsOverview.paidCount} of {paymentsOverview.totalPayments}</span></p>
    </div>
  </CardBody>
}
